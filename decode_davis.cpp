...
// Install signal handler for global shutdown.
	...
	fine = caerBiasCoarseFineGenerate(coarseFineBias); 

	davisHandle.configSet(DAVIS_CONFIG_BIAS, DAVIS240_CONFIG_BIAS_PRBP, fine);	// caerBiasCoarseFineGenerate(coarseFineBias)
	...
	davisHandle.configSet(DAVIS_CONFIG_BIAS, DAVIS240_CONFIG_BIAS_PRSFBP, fine);	//caerBiasCoarseFineGenerate
			...
			if (packet->getEventType() == POLARITY_EVENT) {
				...
				for (const auto &evt : *polarity) {
					if (evt.isValid() == true){
						...
						if (firstpol == true){
							firsttime = polarity_event.timestamp;
							firstpol = false; 
						}
				...
				int n = polarity_events.size();
				printf("Time span of polarity event: %d\n", (polarity_events[n-1].timestamp - firsttime));
